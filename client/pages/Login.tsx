...
export default function Login() {
  ...
  useEffect(() => {
    const saved = typeof window !== 'undefined' ? window.localStorage.getItem('remember_me') : null
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post<AuthLoginResponse>("/api/admin/auth/login", {
      ...
      if (rememberMe) window.localStorage.setItem('remember_me', '1'); else window.localStorage.removeItem('remember_me')

      toast({
        title: "Login Successful!",
        description: `OTP sent to your email. Code: ${res?.data?.otp}`,
      })

      navigate("/verify-otp", { state: { email, otp: res.data.otp, rememberMe } })
    } catch (error: any) {
      const serverMsg: string | undefined = error?.data?.message || error?.message
      const description = serverMsg || "Invalid email or password. Please try again."
      toast({
        title: "Login Failed",
        description,
        variant: "destructive",
            ...
            >
              ...
              >
                <svg className="w-5 h-5 mr-3" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M22 12c0-5.52-4.48-10-10-10S2 6.48 2 12c0 4.99 3.66 9.12 8.44 9.88V15.47h-2.54v-3.47h2.54V10.7c0-2.5 1.49-3.88 3.77-3.88 1.09 0 2.23.2 2.23.2v2.45h-1.25c-1.24 0-1.63.77-1.63 1.56v1.87h2.78l-.44 3.47h-2.34v6.41C18.34 21.12 22 16.99 22 12z" />
                </svg>
