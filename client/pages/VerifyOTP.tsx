...
import type { VerifyOtpResponse, ResendOtpResponse } from "@shared/api"
...
export default function VerifyOTP() {
  ...
  const rememberFromNav = location.state?.rememberMe as boolean | undefined
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post<VerifyOtpResponse>("/api/admin/auth/verify-otp", {
      ...
      const remember = rememberFromNav ?? (typeof window !== 'undefined' && window.localStorage.getItem('remember_me') === '1')
      if (remember) {
        setCookie("access_token", access, { days: 365 })
        if (refresh) setCookie("refresh_token", refresh, { days: 365 })
      } else {
        setCookie("access_token", access)
        if (refresh) setCookie("refresh_token", refresh)
      }
      // Remove pre-verify temporary token
      ...
      const isTokenExpired =
        ...
        typeof serverMsg === "string" && /token\s+expired/i.test(serverMsg)
      ...
      if (isTokenExpired) {
        ...
        toast({
          ...
          description: "Your session token has expired. Please log in again.",
          variant: "destructive",
        })
        navigate("/login", { replace: true, state: { type: "error", message: "Session expired. Please log in again." } })
        return
      }
  ...
  const handleResendOTP = async () => {
    ...
    try {
      ...
      const res = await api.post<ResendOtpResponse>("/api/admin/auth/resend-otp", {
        headers: { Authorization: `Bearer ${token}` },
      })
      ...
      if (isTokenExpired) {
        ...
        toast({
          ...
          description: serverMsg || "Unable to send new code. Please try again.",
          variant: "destructive",
        })
      }
    } finally {
                ...
                >
                  {timeLeft > 0 ? (
                    <p className="text-muted-foreground text-sm">
                      Resend code in <span className={`font-mono font-medium ${timeLeft < 60 ? 'text-orange-500' : 'text-brand-600'}`}>{formatTime(timeLeft)}</span>
                    </p>
                  ) : (
