...
export default function PrivacyPolicy() {
  ...
  const fetchPage = async (page = pageNumber, size = pageSize, q = query) => {
    setLoading(true);
    try {
      const params: Record<string, any> = { documentType: "PRIVACY_POLICY", pageSize: size, pageNumber: page };
      ...
      try {
        const res2 = await api.get<any>("/api/policies", { query: { type: "PRIVACY_POLICY" } });
  ...
  const actuallyDelete = async () => {
    ...
    try {
      ...
      toast({ title: "Delete failed", description: e?.message || "Unable to delete policy", variant: "destructive" });
    } finally {
      setDeleting(false);
    }
  };
  ...
  return (
    <Card className="w-full">
      <CardHeader className="space-y-1">
        <div className="flex items-center justify-between">
          <div>
            ...
            <CardDescription>Manage privacy policy documents.</CardDescription>
          </div>
      ...
      <CardContent>
        <div className="text-sm text-muted-foreground mb-3">All Policies ({countLabel})</div>
              ...
              {loading ? (
                ...
                items.map((p, idx) => (
                  <TableRow key={(p as any).id ?? idx}>
                    ...
                    <TableCell>
                      <Badge variant="secondary">{(p as any).documentType || p.type || "PRIVACY_POLICY"}</Badge>
                    </TableCell>
      ...
      <PolicyFormModal
        ...
        defaultType={(selected as any)?.documentType || (selected as any)?.type || "PRIVACY_POLICY"}
        mode="edit"
      ...
      <Dialog open={imageOpen} onOpenChange={setImageOpen}>
        <DialogContent className="sm:max-w-3xl">
          <DialogHeader>
            <DialogTitle>Policy Image</DialogTitle>
          </DialogHeader>
          {imageUrl ? (
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This action cannot be undone. This will permanently delete the selected policy.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
