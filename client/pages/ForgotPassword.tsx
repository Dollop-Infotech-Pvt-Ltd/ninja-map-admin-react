...
import type { ForgotPasswordStartResponse } from "@shared/api"
...
export default function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await api.post<ForgotPasswordStartResponse>("/api/admin/auth/forget-password", {
      ...
      toast({
        title: "Code Sent!",
        description: `${res?.message || "Check your email for the verification code."} OTP: ${res?.data?.otp}`,
      })

      navigate("/forgot-password/verify", { state: { email, otp: res?.data?.otp } })
    } catch (error: any) {
      const serverMsg: string | undefined = error?.data?.message || error?.message
      setError(serverMsg || "Failed to send verification code. Please try again.")
      toast({
        title: "Failed to Send",
        description: serverMsg || "Unable to send verification code. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsLoading(false)
